...
async function loadBackend() {
  try {
    ...
    if (limits.result_ttl_seconds !== null && limits.result_ttl_seconds !== undefined) {
      ...
      elements.deploymentNote.textContent = `Validated by RIMKit · uploads and results expire after ${minutes} min`;
    }
  } catch (_error) {
...
async function refreshJob() {
  ...
  if (job.status === "succeeded") {
    ...
    showFormError(job.error || "The RIMKit pipeline failed.");
  }
  refreshRunButton();
  return job;
}
...
async function runRetargeting() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(await errorDetail(response, "Could not create a RIMKit job."));
    const job = await response.json();
