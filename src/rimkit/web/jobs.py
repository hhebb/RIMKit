...
from rimkit.pipeline.events import CallbackEventSink, PipelineEvent
...
class _Job:
    ...
    stage: str = "CREATED"
    message: str = "Waiting for the RIMKit worker."
...
class JobManager:
    """Run RIMKit jobs serially while exposing immutable JSON snapshots."""
    ...
    def __init__(
        ...
        *,
        ...
        if max_workers != 1:
            raise ValueError("The RIMKit web adapter currently supports one worker.")
    ...
    def _ensure_capacity_locked(self) -> None:
        ...
        if active >= self.max_active_jobs:
            raise JobCapacityError(
                f"RIMKit is currently processing or queueing {active} job(s); try again later."
            )
    ...
    def _execute(self, job_id: str) -> None:
        ...
        try:
            ...
            LOGGER.exception("RIMKit web job %s failed", job_id)
            with self._condition:
                job = self._get_job(job_id)
