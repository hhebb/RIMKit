"""FastAPI application for the RIMKit browser demo."""
...
from rimkit.motion.source import SourceMotionSummary, validate_source_motion
...
def _safe_original_filename(upload: UploadFile) -> str:
    ...
    if Path(filename).suffix.lower() not in SOURCE_MOTION_SUFFIXES:
        raise HTTPException(
            status_code=415,
            detail="RIMKit accepts Kimodo .npz or GEM-X .pt source motion.",
        )
    return filename
...
def create_app(
    ...
    *,
    ...
    """Create the same-origin RIMKit web application."""
    ...
    static_dir = Path(str(files("rimkit.web").joinpath("static")))
    index_path = Path(str(files("rimkit.web").joinpath("templates").joinpath("index.html")))
    ...
    async def download_example_motion(example_id: str) -> FileResponse:
        ...
        if match is None:
            raise HTTPException(status_code=404, detail="Unknown RIMKit example motion.")
        example, path = match
        return FileResponse(
            path,
            filename=example.filename,
    ...
    async def get_job(job_id: str) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=404, detail="Unknown RIMKit job.") from exc
    ...
    async def download_artifact(job_id: str, artifact_name: str) -> FileResponse:
        try:
            ...
            raise HTTPException(status_code=404, detail="Unknown RIMKit artifact.") from exc
        media_types = {
